...
@dataclass
class MultiSynchronizedSensorData:
    ...
    num_sweeps_in_log: int = None
    timestamp_city_SE3_ego_dict: TimestampedCitySE3EgoPoses = None
...
class MultiSensorData:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cam_names:
            synchronization_cache_path = HOME / ".cache" / "av2" / f"{self.split}_synchronization_cache.feather"
            ...
            if self.with_cache and synchronization_cache_path.exists():
                ...
                self.sensor_cache = self.sensor_cache[self.sensor_cache['split'] == self.split]
            ...
            for src_sensor_name in [self.cam_names[0].value, 'lidar']:
                masked_rows = (self.synchronization_cache['sensor_name'] != src_sensor_name) | (self.synchronization_cache['ring_side_left'].notna()) & (self.synchronization_cache['ring_front_left'].notna()) & (self.synchronization_cache['ring_front_right'].notna()) & (self.synchronization_cache['ring_side_right'].notna()) & (self.synchronization_cache['lidar'].notna())
                
                item = self.synchronization_cache[~masked_rows]
                item['timestamp_ns'] = item.loc[:, src_sensor_name].copy().apply(lambda x: x.asm8.astype(np.int64))
                item = item[["split",  "log_id", "sensor_name", "timestamp_ns"]].copy()

                # We merge and check which rows are the same, meaning they should be removed
                self.sensor_cache = pd.merge(self.sensor_cache, item, indicator=True, how='outer')
                assert (~masked_rows).sum() == (self.sensor_cache['_merge'] == 'both').sum()

                self.sensor_cache = self.sensor_cache[self.sensor_cache['_merge'] != 'both']
                self.sensor_cache = self.sensor_cache.drop(['_merge'], axis=1)
                self.synchronization_cache = self.synchronization_cache[masked_rows]
            ...
            self.synchronization_cache = self.synchronization_cache[self.synchronization_cache['split'] == self.split]
    ...
    def num_sweeps(self) -> int:
        ...
        return int(self.sensor_counts[self.cam_names[0].value]) if self.all_camera_frames else int(self.sensor_counts["lidar"])
    ...
    def _build_sensor_cache(self) -> pd.DataFrame:
        ...
        sensor_cache_path = HOME / ".cache" / "av2" / f"{self.split}_sensor_cache.feather"
        ...
        # sensor_cache = sensor_cache[sensor_cache['split'] == self.split]
    ...
    def populate_lidar_records(self) -> pd.DataFrame:
        ...
        train_paths = (self.dataset_dir / 'train').glob(LIDAR_PATTERN)
        val_paths = (self.dataset_dir / 'val').glob(LIDAR_PATTERN)
        lidar_paths = sorted(chain(train_paths, val_paths), key=lambda x: int(x.stem))
        lidar_record_list = [
    ...
    def populate_image_records(self) -> pd.DataFrame:
        ...
        train_paths = (self.dataset_dir / 'train').glob(CAMERA_PATTERN)
        val_paths = (self.dataset_dir / 'val').glob(CAMERA_PATTERN)
        cam_paths = sorted(chain(train_paths, val_paths), key=lambda x: int(x.stem))
    ...
    def regular_get_item(self, idx: int) -> MultiSynchronizedSensorData:
    ...
    def __getitem__(self, idx: int) -> MultiSynchronizedSensorData:
        """Load the lidar point cloud and optionally the camera imagery and annotations.
        ...
        # Grab the lidar record at the specified index.
        # Selects data at a particular level of a MultiIndex.
        src_sensor_name = self.cam_names[0].value
        record: Tuple[str, str, int] = self.sensor_cache.xs(key=src_sensor_name, level=2).iloc[idx].name
        split, log_id, timestamp_ns = record
        
        src_timestamp_ns = timestamp_ns
        src_timedelta_ns = pd.Timedelta(src_timestamp_ns)
        src_to_target_records = self.synchronization_cache.loc[(split, log_id, src_sensor_name)].set_index(src_sensor_name)
        ...
        target_timestamp_ns = src_to_target_records.loc[src_timedelta_ns, 'lidar']
        if pd.isna(target_timestamp_ns):
            # No match was found within tolerance.
            return None

        target_timestamp_ns = target_timestamp_ns.asm8.item()
        ...
        datum = MultiSynchronizedSensorData(
            log_id=log_id,
            timestamp_ns=timestamp_ns,
            lidar_timestamp_ns=target_timestamp_ns,
            split=split
        )

        # Load camera imagery if enabled.
        if self.cam_names:
            datum.synchronized_imagery = self._load_synchronized_cams(split, sensor_dir, log_id, timestamp_ns, src_sensor_name=src_sensor_name)
    ...
    def find_closest_target_fpath(
        ...
        if src_sensor_name == target_sensor_name:
            timestamp_ns_str = str(src_timestamp_ns)
        else:
            src_timedelta_ns = pd.Timedelta(src_timestamp_ns)
    ...
    def _load_synchronized_cams(
        self, split: str, sensor_dir: Path, log_id: str, sweep_timestamp_ns: int, src_sensor_name = "lidar"
        ...
        cam_paths = [
            self.find_closest_target_fpath(
                split=split,
                log_id=log_id,
                src_sensor_name=src_sensor_name,
        ...
        for p in cam_paths:
            if p is not None:
                cams[p.parent.stem] = TimestampedImage(
                    img=read_img(p, channel_order="RGB"),
