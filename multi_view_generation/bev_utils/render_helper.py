...
def render_map_in_image(nusc,
        ...
        # Default layers.
        if layer_names is None:
            layer_names = ['road_segment', 'lane', 'ped_crossing', 'walkway',
                           'stop_line', 'carpark_area']

        # Check layers whether we can render them.
        for layer_name in layer_names:
            assert layer_name in nusc_map.non_geometric_polygon_layers, \
                'Error: Can only render non-geometry polygons: %s' % layer_names
        ...
        assert nusc_map.map_name == log_location, \
            ...
            nusc_map.map_name, log_location)
        ...
        im = Image.fromarray(draw_image, mode='RGB')
        # im2 = Image.open(cam_path)
        im_size = im.size
        cs_record = nusc.get('calibrated_sensor',
                             cam_record['calibrated_sensor_token'])
        cam_intrinsic = np.array(cs_record['camera_intrinsic'])

        cam_intrinsic[0][2] -= 350
        ...
        records_in_patch = nusc_map.get_records_in_patch(box_coords, layer_names,
                                                     'intersect')
        ...
        for layer_name in layer_names:
            for token in records_in_patch[layer_name]:
                ...
                for polygon_token in polygon_tokens:
                    ...
                    label = layer_name
                    ax.add_patch(descartes.PolygonPatch(polygon_proj,
                                                        fc=color_map[
                                                            layer_name],
                                                        alpha=alpha,
                                                        label=label))
        ...
        for i, bbx in enumerate(boxes):
            points = view_points(bbx.corners(), cam_intrinsic, normalize=True)[:2, :]
            max_x = np.max(points.T, axis=0)[0]
            max_y = np.max(points.T, axis=0)[1]
            min_x = np.min(points.T, axis=0)[0]
            min_y = np.min(points.T, axis=0)[1]
            polygon_proj = Polygon([(min_x, min_y), (max_x, min_y), (max_x, max_y), (min_x, max_y)])
            
            
            # if polygon_proj.area > max_area:
            #     max_area = polygon_proj.area
            #     max_poly = polygon_proj

            
            #if max_poly is not None:
            label = layer_name
            ax.add_patch(descartes.PolygonPatch(polygon_proj,
                                                    fc=rgb2hex(*classname_to_color[bbx.name]),
                                                    alpha=0.5,
                                                    label=label))
